...
export default class Waiver extends UIComponent {
  ...
  render() {
    const { waiver, permitWording, addableWaivers = [], addableWaiversLoaded } = this.props;
            ...
            <div className="panel">
              <div className="waiver">
                <h3 className="sectionTitle">
                  ...
                  <span className={`update-changed-label ${hasNew === 'true' ? '' : 'u-hidden'}`}>UPDATED</span>
                  <i
                    className={`icon ${this.state.displayWaiverList ? 'icon-chevron-up' : 'icon-chevron-down'}`}
                    onClick={() => { this.toggleWaiverList(); }}
                  />
                </h3>
                <div className={`${this.state.displayWaiverList ? '' : 'u-hidden'}`}>
                  <div className="header-section">
                    <div>
                      <span>WAIVER DESCRIPTION</span>
                      {
                        ...
                        <span
                          className={classNames('header-section__add-link', { 'is-disabled': !addableWaiversLength })}
                  ...
                  <div
                    className={`dataList ${!data.size && addableWaiversLength && !addableWaiversLoaded ? 'empty' : ''}`}
                  >
                    {
                      data.map((item, i) => {
