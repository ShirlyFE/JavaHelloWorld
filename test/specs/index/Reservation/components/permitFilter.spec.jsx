...
describe('index/Reservation/components/PermitFilter', () => {
  ...
  it('Reservation Components should render without errors', () => {
    ...
    expect(component.find('.searchtag')).toHaveLength(1);

    component.find('.clearFilter').simulate('click');

    const wrapDropdown = component.find(Dropdown);
    expect(wrapDropdown.at(0).hasClass('search-type-dropdown')).toBeTruthy();
    expect(wrapDropdown.at(0).node.props.defaultValue).toEqual(PermitSearchType.PERMIT_NUMBER);
    wrapDropdown.at(0).node.props.onChange({ value: PermitSearchType.PERMIT_NUMBER });
    wrapDropdown.at(0).node.props.onChange({ value: PermitSearchType.CUSTOMER_NAME });
  ...
  it('When displayFilter equal to false should render without errors', () => {
    const mockStore = configureStore();
    const store = mockStore({});
    ...
    expect(component.find('.icon-chevron-down')).toHaveLength(2);
    expect(component.find('.filter')).toHaveLength(0);
