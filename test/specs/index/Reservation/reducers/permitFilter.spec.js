...
describe('index -> reservation -> reducers -> permitFilter', () => {
  ...
  it('should return the initial state', () => {
    ...
    expect(state.get('displayFilter')).toBe(false);
  ...
  it('CHANGE_FILTER_DISPLAY should work well', () => {
    let state = permitFilterReducer(undefined, {
      type: actions.CHANGE_FILTER_DISPLAY,
      payload: {
        value: true
      }
    });

    expect(state.get('displayFilter')).toBe(true);

    state = permitFilterReducer(undefined, {
      type: actions.CHANGE_FILTER_DISPLAY,
      payload: {
        value: false
      }
