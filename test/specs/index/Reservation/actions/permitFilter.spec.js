...
describe('index -> reservation -> actions -> permitFilter', () => {
  ...
  it('showFilterAction should works fine', () => {
    const action = actions.showFilterAction();

    expect(typeof action).toBe('object');
    expect(action.type).toBe(actions.CHANGE_FILTER_DISPLAY);
  });

  it('hideFilterAction should works fine', () => {
    const action = actions.hideFilterAction();
