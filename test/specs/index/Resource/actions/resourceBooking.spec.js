...
describe('index/Resource/actions/resourceBooking', () => {
  ...
  it('fetchResourcesBooking should work fine if it\'s changing date', () => {
    const { fetchResourcesBooking } = actions;
    const isChangeDate = true;
    return store.dispatch(fetchResourcesBooking({}, isChangeDate))
      .then(() => {
        const storeActions = store.getActions();
  ...
  it('setResourceSelectedDate should work fine', () => {
    const { SET_RESOURCE_SELECTED_DATE, setResourceSelectedDate } = actions;

    store.dispatch(setResourceSelectedDate(''));
    const action = first(store.getActions());
