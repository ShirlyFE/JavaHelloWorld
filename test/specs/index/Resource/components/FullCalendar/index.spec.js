...
describe('index/Resource/components/FullCalendar', () => {
  ...
  test('updateFixedElementsPosition', () => {
    const component = setupRaw(props);
    const instance = component.instance();
    const mockFcHeader = document.createElement('div');
    mockFcHeader.classList.add('fc-header');
    document.body.appendChild(mockFcHeader);

    instance.updateFixedElementsPosition();
    expect(mockFcHeader.style.left).toBe('');
    ...
    instance.updateFixedElementsPosition();
    expect(mockFcHeader.style.left).toBe('14px');

    window.pageXOffset = 10;
    instance.updateFixedElementsPosition();
    expect(mockFcHeader.style.left).toBe('4px');

    window.pageXOffset = 20;
    instance.updateFixedElementsPosition();
    expect(mockFcHeader.style.left).toBe('-6px');
  });
