...
describe('index/resource/components/BookingInformation/BookingDetails', () => {
  it('render booking detail items', () => {
    ...
    expect(component.find('.data-box').length).toBe(3);
    expect(component.find('.date')).toHaveLength(6);
    expect(component.find('.dashed')).toHaveLength(3);
    expect(component.find('.time')).toHaveLength(6);
    expect(component.find('.icon')).toHaveLength(21);
  ...
  it('render date range booking detail items', () => {
    const props = {
      ...
      resourceItem: fromJS({
        ...
        resourceType: 2,
        definedDateRange: [
          { id: 3, text: '2017 Jan 03 to 2017 Jan 06', value: 3 },
          { id: 4, text: '2017 Jan 07 to 2017 Jan 10', value: 4 },
          { id: 5, text: '2017 Jan 11 to 2017 Jan 16', value: 5 }
        ],
        bookingDetail: [
          {
            dateRangeID: 3,
    ...
    expect(component.find('.data-box').length).toBe(2);
    expect(component.find('.date')).toHaveLength(0);
    expect(component.find('.dashed')).toHaveLength(0);
    expect(component.find('.time')).toHaveLength(0);
    expect(component.find('.aaui-dropdown__button-text')).toHaveLength(2);
    expect(component.find('.icon')).toHaveLength(8);
  ...
  it('render rental block booking detail items', () => {
    const props = {
      ...
      detailsError: [{ pendingID: 'pending_775_7161', resourceBookingID: 0, errors: {} }],
    ...
    expect(component.find('.detail-item')).toHaveLength(2);
    expect(component.find('.data-box').length).toBe(2);
    expect(component.find('.date')).toHaveLength(2);
    expect(component.find('.dashed')).toHaveLength(0);
    expect(component.find('.time')).toHaveLength(0);
    expect(component.find('.aaui-dropdown__button-text')).toHaveLength(2);
    expect(component.find('.icon')).toHaveLength(12);
    expect(component.find('.icon-people-m')).toHaveLength(2);
    expect(component.find('.icon-repeat-m')).toHaveLength(2);
    expect(component.find('.icon-close')).toHaveLength(2);
