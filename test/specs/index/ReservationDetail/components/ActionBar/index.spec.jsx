...
describe('index -> ReservationDetail -> components -> ActionBar', () => {
  ...
  it('should include 4 buttons 1 dropdown', () => {
    Authority.init(window.__authoritiy__);
    const { actionBar } = setup();
    expect(actionBar.find('.actions').find('.btn').length).toBe(3);
  ...
  it('should hide buttons when authorities are "hide"', () => {
    ...
    const { actionBar } = setup();
    expect(actionBar.find('.actions').find('.btn.u-hidden').length).toBe(3);
  });
  ...
  it('Handle permit status change', () => {
    ...
    const alerts = actionBar.find('Alert');
    const denyAlert = alerts.at(0);
    const completeAlert = alerts.at(1);

    expect(completeAlert.find('.modal').hasClass('is-open')).toBe(false);
    instance.setStatus({ value: 7 }) // Complete
    expect(completeAlert.find('.modal').hasClass('is-open')).toBe(true);
    expect(actions.changePermitStatusAsycAction.mock.calls.length).toBe(0);

    const completeButton = completeAlert.find('Button').last();

    completeButton.simulate('click');
    expect(completeAlert.find('.modal').hasClass('is-open')).toBe(false);
    expect(actions.changePermitStatusAsycAction.mock.calls.length).toBe(1);

    expect(denyAlert.find('.modal').hasClass('is-open')).toBe(false);
    instance.setStatus({ value: 9 }) // Deny
    expect(denyAlert.find('.modal').hasClass('is-open')).toBe(true);

    const denyButton = denyAlert.find('Button').last();
    ...
    expect(actions.changePermitStatusAsycAction.mock.calls.length).toBe(3);
  });
