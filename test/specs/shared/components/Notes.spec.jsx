...
describe('shared => components => Notes', () => {
  it('should rendered correctly', () => {
    const { wrapper } = setup();
    expect(wrapper.find('div.panel.u-hidden')).toHaveLength(1);

    wrapper.setProps({ notes: notes.set('showSection', true)});
    expect(wrapper.find('div.panel.u-hidden')).toHaveLength(0);
  ...
  it('should expanded and collapsed correctly', () => {
    const { wrapper, instance } = setup();
    expect(wrapper.find('div.section-container.u-hidden')).toHaveLength(1);

    wrapper.setProps({ notes: notes.set('isExpand', true)});
    expect(wrapper.find('div.section-container.u-hidden')).toHaveLength(0);

    wrapper.setProps({ notes: notes.set('isExpand', false)});
    expect(wrapper.find('div.section-container.u-hidden')).toHaveLength(0);

    instance.onToggle()
    expect(wrapper.find('div.section-container.u-hidden')).toHaveLength(1);

    wrapper.find('i.title.icon').simulate('click')
    expect(wrapper.find('div.section-container.u-hidden')).toHaveLength(0);
  });
