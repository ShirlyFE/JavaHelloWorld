...
describe('shared/actions/cancelPermit', () => {
  ...
  it('gotoRefundDepositFeesPageAction method works fine', () => {
    ...
    expect(storeActions[1].payload).toEqual({
      url: '/index.RefundDeposits.html?&permit_id=undefined&batch_id=0&receipt_id=0'
    });
  });
  ...
  it('gotoReservationDetailPageAction method works fine', () => {
    ...
    expect(storeActions[1].type).toEqual('LOCATION_REDIRECT');
    expect(storeActions[1].payload).toEqual({
      url: '/index.ReservationDetail.html?&permit_id=1',
      win: null,
      useReplace: false
    });
  });
  ...
  it('reEnterReservationDetailPageAction method works fine', () => {
    ...
    expect(storeActions[1].type).toEqual('LOCATION_REDIRECT');
    expect(storeActions[1].payload).toEqual({
      url: '/index.ReservationDetail.html?&batch_id=0&receipt_id=0'
    });
  });
    ...
    expect(mockConfirm).toBeCalledWith(
      [
        ...
        <div className="action-message-warning">
          <span className="aaui-alert-warning-icon icon-exclamation" /> Cancelled reservation cannot be modified. Proceed?
        </div>
      ],
      {
        cancelText: "No",
  ...
  it('confirmOnlyCancelPermit method works fine', () => {
    ...
    expect(mockConfirm.mock.calls[0]).toEqual([
      [<div>Are you sure you want to cancel this {permit.toLocaleLowerCase()}?</div>, <div className="action-message-warning"><span className="aaui-alert-warning-icon icon-exclamation" /> Cancelled reservation cannot be modified. Proceed?</div>], {"cancelText": "No", "confirmText": "Yes", "showCancel": true, "title": "Cancel Permit"}
    ]);
  });
